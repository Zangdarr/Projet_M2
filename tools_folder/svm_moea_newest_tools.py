...
def runTcheby():
    ...
    newest_decisions        = copy.deepcopy(best_decisions)
    newest_decisions_scores = copy.deepcopy(best_decisions_scores)
    ...
    for itera in range(nb_iterations):
        #Update model
        training_input, training_output, discard_cmpt, training_scores = train_to.getTrainingSet(model_directions, newest_decisions, newest_decisions_scores ,z_opt_scores, strategy, nb_functions, training_neighborhood_size)
        newest_decisions = []
        ...
        if(writeR2OK):
            ...
            kf = cross_validation.KFold(n=pop_size-discard_cmpt, n_folds=10, shuffle=True,
                                           random_state=None)
        ...
        for f in id_directions:
            ...
            newest_decisions.append(new_input)
            newest_decisions_scores.append(mix_scores)
            newest_len += 1
