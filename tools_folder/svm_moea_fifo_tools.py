...
def runTcheby():
    ...
    #FIFO for training offspring
    fifo_offspring = Fifo(FIFO_SIZE)
    #FIFO for training score offspring
    fifo_score     = Fifo(FIFO_SIZE)
    #FIFO for direction associated to offspring
    fifo_direction = Fifo(FIFO_SIZE)

    for i in range(nb_functions):
        fifo_offspring.push_in(best_decisions[i])
        fifo_score.push_in(best_decisions_scores[i])
        fifo_direction.push_in(i)
    ...
    for itera in range(nb_iterations):
        ...
        if(not free_eval):
            #Update model
            training_inputs, training_outputs, training_set_size, training_scores = train_to.getFifoBasedTrainingSet(model_directions, fifo_offspring.get(), fifo_score.get(), fifo_direction.get() ,eval_to.getZstar_with_decal(), strategy, fifo_offspring.get_nbr_elements(), training_neighborhood_size)
        ...
        for f in id_directions:
            ...
            for j in f_neighbors:
                ...
                #insert in FIFO for training
                fifo_offspring.push_in(best_candidate)
                fifo_score.push_in(mix_scores)
                fifo_direction.push_in(j)
