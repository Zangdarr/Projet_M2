...
import multimodel_filtring_tools as filt_to
...
import multimodel_training_tools as train_to
import io_tools as iot
import multimodel_quality_tools as qual_tools
...
def runTcheby():
    ...
    #table containing one model for each objective of the problem considered
    model_tab = [NuSVR() for i in range(nb_objectives)]
    ...
    qual_tools.resetGlobalVariables(filenameDIR, filenameSCORE, nb_iterations, nb_functions, nb_objectives)
    ...
    if(file_to_writeR2 != NO_FILE_TO_WRITE):
        ...
        for i in range(nb_objectives):
            newfilenameR2 = "R2-model"+ str(i+2)+filenameDIR[3:]
    ...
    for itera in range(nb_iterations):
        if(not free_eval):
            #Update model
            training_inputs, training_outputslist, training_set_size = train_to.getTrainingSet(best_decisions, best_decisions_scores)
            train_to.multimodel_fit(model_tab, training_inputs, training_outputslist)
        ...
        if(writeR2OK and not free_eval):
            ...
            MAE_cv = [[] for i in range(0, nb_objectives)]
            MDAE_cv = [[] for i in range(0, nb_objectives)]

            clfCV = [NuSVR() for i in range(0, nb_objectives)]
            ...
            for fold_size in folds_sizes:
                ...
                for i in range(0, nb_objectives):
                    clfCV[i].fit(training_inputs_array[random_index[mask]], training_outputslist[i][random_index[mask]])

                    test_fold_tcheby = training_outputslist[i][random_index[start:stop]]
                    test_fold_predict = clfCV[i].predict(training_inputs_array[random_index[start:stop]])

                    R2_cv[i]  .append(r2_score             (test_fold_tcheby, test_fold_predict))
                    MSE_cv[i] .append(mean_squared_error   (test_fold_tcheby, test_fold_predict))
                    MAE_cv [i].append(mean_absolute_error  (test_fold_tcheby, test_fold_predict))
                    MDAE_cv[i].append(median_absolute_error(test_fold_tcheby, test_fold_predict))

            R2 = [clfCV[i].score(training_inputs, training_outputslist[i]) for i in range(0,nb_objectives)]
            MSE_cv_mean  = [numpy.mean(MSE_cv[i])     for i in range(0, nb_objectives)]
            RMSE_cv_mean = [math.sqrt(MSE_cv_mean[i]) for i in range(0, nb_objectives)]
            MAE_cv_mean  = [numpy.mean(MAE_cv[i])     for i in range(0, nb_objectives)]
            MDAE_cv_mean = [numpy.mean(MDAE_cv[i])    for i in range(0, nb_objectives)]
            R2_cv_mean   = [numpy.mean(R2_cv[i])      for i in range(0, nb_objectives)]

            iot.multimodel_printR2(file_to_writeR2_LIST, eval_to.getNbEvals(), itera,  R2, R2_cv_mean, MSE_cv_mean , MAE_cv_mean, MDAE_cv_mean, RMSE_cv_mean, problem_size, nb_objectives, print_every=1)
        ...
        for f in id_directions:
            ...
            filter_param = [itera, f, model_tab, f_neighbors, list_offspring, model_directions, start_fct, problem_size, eval_to.getZstar_with_decal(), best_decisions_scores, best_decisions, nb_objectives]
            best_candidate = filt_to.filter_function(filter_strat, filter_param)

            #evaluation of the newly made solution
            mix_scores = eval_to.eval(start_fct, best_candidate, problem_size)

            #MAJ of the z_star point, no use to has_changed for this version
    ...
    if(not free_eval and writeR2OK):
        ...
        for i in range(1, nb_objectives):
            file_to_writeR2_LIST[i].close()
    return
