...
def getProblemTitle():
    return "Unconstrained Problem 10"
...
def y_function(x_1, x_2, x_j, j, n):
    return x_j - 2 * x_2 * math.sin(2 * math.pi * x_1 + (j * math.pi / n))
...
def f1(x_vector, vector_size):
    ...
    for j in range(3, vector_size, 3):
        tmp_y = y_function(x_1, x_2, x_vector[j], j, vector_size)
        sum_y += (4 * (tmp_y)**2  - math.cos(8 * math.pi * tmp_y) + 1)


    return tmp1 * tmp2 + tmp3 * sum_y
